...
def get_utxo_from_private_keys(
    coin,
    wif_private_keys,
    is_script=False,
...
def send_utxos(coin, utxos, destination_address, satoshis, fee):
    ...
    for utxo in utxos:
        ...
        raise Exception(
            'Not enough funds in address(es).\n'
            'Sum of inputs is {} which is less than {} ({} + {})'.format(
                input_satoshis, satoshis + fee, satoshis, fee
            )
        )
