...
def test_der_encode_signature():
    ...
    expected_signature = (
        '30440220059c71db0d01d284fc4589c03b09947d21b98d7cefae614ac1039b6de0fa28b'
...
def test_encode_decode_points():
    ...
    for i in range(100):
        assert random_point == decode_point(
            encode_point(random_point, compressed=random.choice([True, False]))
        )
        assert random_point == decode_point(
            encode_point(random_point, compressed=random.choice([True, False])).hex()
        )
        random_point *= 10
